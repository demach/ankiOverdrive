...
def discovery_driving_policy(self, **kwargs):
    def clockwise_format(bool_str):
        if bool_str.lower() == "true":
            return "cw_1"
        else:
            return "cw_0"
    def create_trackfile_insert(track_pieces_lst):
        write_lst = []
    ...
    if not hasattr(self, 'last_abspos'):
        self.last_abspos = (int(kwargs["piece"]), int(kwargs["location"]), kwargs["is_clockwise"], "dummy_element")
    
    if self.last_abspos != (int(kwargs["piece"]), int(kwargs["location"]), kwargs["is_clockwise"]):
        #new absolute car position detected
        if self.track_pieces == [] and int(kwargs["piece"]) == 34 and kwargs["is_clockwise"] == "False":
            car.turn()
            car.change_lane(400, 2000, -68)
            self.track_pieces.append((34, "cw_1"))
            self.track_pieces.append((33, "cw_1"))
        elif self.track_pieces == [] and int(kwargs["piece"]) == 34 and kwargs["is_clockwise"] == "False":
            self.track_pieces.append((34, "cw_1"))
            self.track_pieces.append((33, "cw_1"))
        elif (33, "cw_1") in self.track_pieces:
            if int(kwargs["piece"]) != self.track_pieces[-1][0] or kwargs["is_clockwise"] != self.last_abspos[2]:
                #new piece number or
                #direction_change
                self.track_pieces.append((int(kwargs["piece"]), clockwise_format(kwargs["is_clockwise"])))
            elif int(kwargs["piece"]) == self.track_pieces[-1][0] and kwargs["is_clockwise"] == self.last_abspos[2]:
                #same piece number, but new counter
                if clockwise_format(kwargs["is_clockwise"]) == "cw_1" and int(kwargs["location"]) < self.last_abspos[1]:
                    self.track_pieces.append((int(kwargs["piece"]), clockwise_format(kwargs["is_clockwise"])))
                elif clockwise_format(kwargs["is_clockwise"]) == "cw_0" and int(kwargs["location"]) > self.last_abspos[1]:
                    self.track_pieces.append((int(kwargs["piece"]), clockwise_format(kwargs["is_clockwise"])))
